...
def AbuseIPDB_check(IP):
    ...
    Usage = parsed_data["data"]["usageType"]
    ...
    return total_reports, abuse_confidence_score, domain, human_readable_time, Country_Name, Usage
                ...
                if is_valid_ip(IP_address):
                    if ping(IP_address):
                        ...
                        if not AbuseIPDB_API_KEY == "":
                            total_reports, abuse_confidence_score, domain, human_readable_time, Country_Name, Usage = \
                                AbuseIPDB_check(IP_address)
                            print("\n--- AbuseIPDB Results ---")
                        ...
                        if Missing_API_Counter > 0:
                            print(f"Please note that you are missing {Missing_API_Counter} API keys\nIt is recommended"
                                  f" to use all of the "
                        ...
                        if Check_Validation == "N":
                            ...
                            if Missing_API_Counter > 0:
                                print(
                                    f"Please note that you are missing {Missing_API_Counter} API keys\n\nIt is recommended"
                                    f" to use all of the "
                                    "available keys for a better experience.\n")
                                print("Please notice that the IP address was not reachable using ping.\n")
                                Continue = input("\nCheck another IP?(Y/N)\n")
                                if Continue == "N":
                                    break
                                else:
